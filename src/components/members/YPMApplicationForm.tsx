...
import {
  ypmFormSchema,
  type YPMFormData,
} from "@/lib/validation/ypmFormSchema";
...
export default function YPMApplicationForm() {
  ...
  const handleQualificationChange = (
    ...
    if (file.size > MAX_QUALIFICATION_SIZE) {
      ...
      setQualificationFile(null);
      if (qualificationInputRef.current)
  ...
  const getInputClasses = (hasError: boolean) =>
    `w-full text-zinc-700 px-4 py-3 rounded-lg border-2 transition-all duration-200 focus:outline-none ${
      hasError
    ...
    return (
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-zinc-100 text-center">
        ...
        <p className="text-zinc-600 mb-6 max-w-md mx-auto">
          Thank you for applying for Yoga Professional Membership. We will
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
        ...
        <div>
          <label className="block text-sm font-semibold text-zinc-700 mb-2">
            Attach Completed Professional Qualification{" "}
          ...
          <p className="mt-2 text-xs text-zinc-500 italic">
            Certificate / Diploma / Degree / Master Degree / PhD in Yoga,
            Max-5MB
          </p>
          {qualificationError && (
            <p className="mt-1 text-sm text-red-600 flex items-center gap-1">
